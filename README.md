...
| **`start-metrics-generation-v1`** | **Legacy**: metrics from **Code + Browser** audits only, with **client-side** metric rules in the prompt (long). Prefer **`start-metrics-generation`** + `compute-metrics`. |
