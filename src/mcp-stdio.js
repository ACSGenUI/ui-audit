...
server.registerTool(
  ...
  async ({ projectName, appUrl, repoUrl, auditorName, auditDate }) => {
    const { parse: csvParse } = await import('csv-parse/sync');
    const warnings = [];

    const loadChecklist = async (templateName) => {
      const filename = config.templates[templateName];
      const filePath = resolve(config.workspaceDir, filename);
      try {
        ...
        warnings.push(`${templateName} not found at ${filePath} — skipped.`);
        return [];
      }
    };

    const [codeRows, browserRows, manualRows] = await Promise.all([
      loadChecklist('code-audit'),
      loadChecklist('browser-audit'),
      loadChecklist('manual-audit'),
    ]);
